  ...
  return (
    <footer className="bg-leather text-ivory">
      <Container className="py-16 text-center">
        ...
        <p className="mt-3 font-heading text-[0.68rem] tracking-[0.28em] uppercase text-gold-deep">
          {site.motto}
        </p>
        <div className="mt-6">
          <Ornament />
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-x-6 gap-y-3">
          {footerNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="font-heading text-[0.68rem] tracking-[0.2em] uppercase text-ivory/80 hover:text-gold-deep"
            >
              {item.label}
            </Link>
          ))}
